...
type PostRow = { id: string; slug: string; title: string; published: boolean; published_at: string };
  ...
  return (
    <AdminShell>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, gap: 16, flexWrap: 'wrap' }}>
        <h1 style={{ fontFamily: 'var(--font-syne), sans-serif', fontSize: 32, fontWeight: 800, letterSpacing: '-0.5px' }}>Journal</h1>
        <Link href="/admin/blog/new" className="btn-primary" style={{ textDecoration: 'none' }}>New post</Link>
      </div>
      {error ? <p style={{ color: '#b91c1c' }}>{error}</p> : null}
      <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
        {rows === null ? <p style={{ padding: 24, color: 'var(--mid)' }}>Loading…</p> : rows.length === 0 ? <p style={{ padding: 24, color: 'var(--mid)' }}>No posts yet.</p> : (
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} style={{ borderTop: '1px solid var(--border)' }}>
                  <td style={td}><Link href={`/admin/blog/${r.id}`} style={{ color: 'inherit', fontWeight: 600 }}>{r.title}</Link></td>
                  <td style={{ ...td, color: 'var(--mid)' }}>{r.slug}</td>
                  <td style={{ ...td, color: 'var(--mid)' }}>{new Date(r.published_at).toLocaleDateString()}</td>
                  <td style={td}>
