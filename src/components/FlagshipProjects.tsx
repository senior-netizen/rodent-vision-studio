...
const projectImages: Record<string, { src: string; alt: string; fit?: "cover" | "contain" }> = {
  "squirrel-api-studio": { src: heroDashboard, alt: "API studio dashboard interface" },
  shedsense: { src: lightsOutMap, alt: "Power visibility map interface" },
  trustchain: { src: rodentLogo, alt: "Rodent brand mark for fintech infrastructure", fit: "contain" },
  "rodent-labs-vawt": { src: heroDashboard, alt: "Monitoring dashboard representing hardware telemetry" },
  "lights-out": { src: lightsOutSchedule, alt: "Power outage schedule planning interface" },
};
    ...
    <section id="flagships" className="section-padding-sm relative overflow-hidden" ref={ref}>
      ...
      <div className="container mx-auto relative z-10">
        ...
        >
          ...
          <Link to="/projects" className="inline-flex items-center gap-2 neumorphic-button text-sm">
            See all projects
          ...
          {featured.map((project, index) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  "group glass-card p-5 sm:p-6 space-y-5 hover:-translate-y-1",
                  isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                )}
                data-tilt-card
